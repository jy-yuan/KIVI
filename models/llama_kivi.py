...
from quant.new_pack import triton_quantize_and_pack_along_last_dim, triton_pack_along_last_dim
from quant.matmul import cuda_bmm_fA_qB_outer
from quant.rope_embedding import fused_rope_and_quant_prefill
...
class LlamaAttention_KIVI(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        raise NotImplementedError("We currently do not implement KIVI over the vanilla attention. Please use the Flashattention version")
...
class LlamaFlashAttention_KIVI(LlamaAttention_KIVI):
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            attn_output, packed = self.decoding(past_key_value, query_states, key_states, value_states, 
                                                  attention_mask, bsz, q_len, kv_seq_len)
            ...
            if key_states.shape[-2] % self.residual_length != 0:
                if key_states.shape[-2] < self.residual_length:
                    ...
                    key_states_full = key_states[:, :, -(key_states.shape[-2] % self.residual_length):, :]
            else:
    ...
    def decoding(self, past_key_value, query_states, key_states, value_states, attention_mask,
        ...
        if key_states_quant_trans is not None:
            attn_weights = torch.empty((bsz, self.num_heads, q_len, kv_seq_len), 
            ...
            attn_weights[:, :, :, :quant_key_len] = cuda_bmm_fA_qB_outer(self.group_size, query_states, key_states_quant_trans_repeat, 
            ...
            attn_weights[:, :, :, quant_key_len:] = torch.matmul(query_states, key_states_full_repeat.transpose(2, 3))
            attn_weights /= math.sqrt(self.head_dim)
        else:
            attn_weights = torch.matmul(query_states, key_states_full_repeat.transpose(2, 3)) / math.sqrt(self.head_dim)

        if key_states_full.shape[-2] == self.residual_length:
            assert self.residual_length % self.group_size == 0
            key_states_quant_trans_new, key_scale_trans_new, key_mn_trans_new = triton_quantize_and_pack_along_last_dim(key_states_full.transpose(2, 3), 
                                                                                                                        self.group_size, 
                                                                                                                        self.k_bits)
            key_states_full = None
            if key_states_quant_trans is not None:
        ...
        if value_full_length > self.residual_length:
            ...
            value_states_quant_new, scale, mn = triton_quantize_and_pack_along_last_dim(value_states_full[:, :, :1, :], 
                                                                                        self.group_size, 
                                                                                        self.v_bits)
            value_states_full = value_states_full[:, :, 1:, :]
            if value_states_quant is not None:
                ...
                value_scale = scale
                value_mn = mn
        return attn_output, (key_states_quant_trans, key_states_full, key_scale_trans, key_mn_trans, \
