...
from quant.new_pack import unpack_and_dequant_vcache, quant_and_pack_vcache, \
    quant_and_pack_kcache, unpack_and_dequant_kcache, triton_quantize_and_pack_along_last_dim
...
class FalconAttention_KIVI(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_states = key_states.transpose(1, 2).reshape(bsz, num_kv_heads, q_len, self.head_dim)
        value_states = value_states.transpose(1, 2).reshape(bsz, num_kv_heads, q_len, self.head_dim)

        kv_seq_len = key_states.shape[-2]
        if layer_past is not None:
            # kv_seq_len += layer_past[0].shape[-2]
            kv_seq_len += layer_past[-1]
        if alibi is None:
            cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)

        if layer_past is not None:
            key_states_quant_trans = layer_past[0]
            key_states_full = layer_past[1]
            key_scale_trans = layer_past[2]
            key_mn_trans = layer_past[3]
            value_states_quant = layer_past[4]
            value_states_full = layer_past[5]
            value_scale = layer_past[6]
            value_mn = layer_past[7]

            if key_states_quant_trans is not None:
                att_qkquant = cuda_bmm_fA_qB_outer(self.group_size, query_states, key_states_quant_trans, 
                                key_scale_trans, key_mn_trans, self.k_bits, mqa=True)
            else:
                att_qkquant = None
            ...
            if value_states_quant is None:
                ...
                attn_output = cuda_bmm_fA_qB_outer(self.group_size, attn_weights[:, :, :, :-value_full_length], value_states_quant, 
                                            value_scale, value_mn, self.v_bits, mqa=True)
        ...
        present = (key_states_quant_trans, key_states_full, key_scale_trans, key_mn_trans, value_states_quant, value_states_full, value_scale, value_mn, kv_seq_len) if use_cache else None
...
class FalconFlashAttention2(FalconAttention_KIVI):
    """
...
class FalconDecoderLayer_KIVI(nn.Module):
    def __init__(self, config: FalconConfig):
        super().__init__()
        hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads
...
class FalconModel_KIVI(FalconPreTrainedModel):
    def __init__(self, config: FalconConfig):
        ...
        # Transformer blocks
        self.h = nn.ModuleList([FalconDecoderLayer_KIVI(config) for _ in range(config.num_hidden_layers)])
...
class FalconForCausalLM_KIVI(FalconPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: FalconConfig):
        super().__init__(config)
        self.transformer = FalconModel_KIVI(config)
...
class LMEvalFalconForCausalLM_KIVI(HFLM):
    ...
    def __init__(
        ...
        # self._config.k_quant_dim = k_quant_dim
        # self._config.v_quant_dim = v_quant_dim
        ...
        if not gptq:
            ...
            self._model = FalconForCausalLM_KIVI.from_pretrained(
                pretrained,
                cache_dir=cache_dir,
                revision=revision,
                config=self._config,
