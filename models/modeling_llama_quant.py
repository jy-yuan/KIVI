...
from transformers.modeling_attn_mask_utils import AttentionMaskConverter, _prepare_4d_causal_attention_mask
from models.utils_quant import cal_tensor_size
from models.utils_quant import cal_tensor_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            # attn_output = cuda_bmm_fA_qB_outer(self.group_size, attn_weights[:, :, :, :-1], value_states_quant, 
            #                                 value_scale, value_mn, self.v_bits)
            # # att_qkquant_our = cuda_bmm_fA_qB_outer(self.group_size, attn_weights[:, :, :, :-1], value_states_quant, 
            # #                                 value_scale, value_mn, self.v_bits)
            # attn_output += torch.matmul(attn_weights[:, :, :, -1:], value_states)
            # value_states_quant_new, scale, mn = triton_quantize_and_pack_along_last_dim(value_states, 
            #                                                                             self.group_size, 
            #                                                                             self.v_bits)
            # value_states_quant = torch.cat([value_states_quant, value_states_quant_new], dim=2)
            # value_scale = torch.cat([value_scale, scale], dim=2)
...
class LMEvalLlamaForCausalLM(HFLM):
    ...
    def __init__(
        ...
        self._config.buffer_length = buffer_length
